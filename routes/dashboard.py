...
oauth2_schema = OAuth2PasswordBearer(tokenUrl="token") # oauth2_schema: Placeholder for token authentication, not actively used in this snippet.
...
@route.get("/dashboard") # Renders the main dashboard showing all jobs by fetching them from JObs_COL FastAPI route decorator
def dashboard(request: Request):#Route handler function
    jobs_list = list(JObs_COL.find({}))#Fetch jobs from MongoDB
    return templates.TemplateResponse("Dashboard.html", {"request": request, "jobs_list": jobs_list})#Render template
...
@route.get("/viewApplications")
def view_applications(request: Request):
    apps = list(APPLICATION_COL.find({}))
    # convert ObjectIds
    for a in apps:
        a["_id"] = str(a["_id"])
...
def serialize_objectid(obj):
    """ Helper f    unction to convert ObjectId to string for JSON serialization """
    if isinstance(obj, ObjectId):
        return str(obj)
    elif isinstance(obj, list):
        return [serialize_objectid(item) for item in obj]
...
@route.get("/joblist") # Renders the main dashboard showing all jobs by fetching them from JObs_COL
def joblist(request: Request):
...
async def hrDashboard(request: Request, data: dict):
    print(data)
